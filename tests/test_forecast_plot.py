...
df_forecast_facet = pd.concat(
    [df_forecast_p1, df_forecast_p2, df_forecast_p3, df_forecast_p4, df_forecast_p5],
    sort=False,
    ignore_index=True,
)
...
class TestForecastPlot(PandasTest):
    def test_plot_forecast_png(self):
        ...
        result = forecast_plot.plot_forecast(
            df_forecast, "png", path, 900, 600, "Test Plot", show_legend=False, auto_open=False
        )
    ...
    def test_plot_forecast_html(self):
        ...
        forecast_plot.plot_forecast(
            df_forecast, "html", path, 600, 400, "Test Plot", show_legend=False, auto_open=False
        )
        self.assertTrue(os.path.isfile(f"{path}.html"))

        path = get_file_path(base_folder, "test_plotly_legend")
        forecast_plot.plot_forecast(
            df_forecast, "html", path, 600, 400, "Test Plot", show_legend=True, auto_open=False
        )
        self.assertTrue(os.path.isfile(f"{path}.html"))
    ...
    def test_plot_forecast_jupyter(self):
        if (not forecast_plot._plotly_imported) or (not forecast_plot._ipython_imported):
            self.skipTest("Test skipped as either plotly or IPython is not installed...")
