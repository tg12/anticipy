...
class TestModelUtils(PandasTest):
    ...
    def test_get_s_x_extrapolate(self):
        ...
        l_df_y = [
            # Single ts
            pd.Series(index=pd.date_range("2016-01-01", periods=10, freq="W"), data=np.arange(10)),
            # Multiple ts
            pd.DataFrame(
                index=pd.date_range("2016-01-01", periods=10, freq="W"),
        ...
        for df_type, model, time_resolution in itertools.product(
            ...
            dict_df = {
                # Single ts
                "single": pd.Series(
                    index=pd.date_range("2016-01-01", periods=10, freq=time_resolution),
                    data=np.arange(10),
                ),
                # Multiple ts
                "multi": pd.DataFrame(
            ...
            logger.info(
                "type of df: %s, f_model: %s , time_resolution: %s", df_type, model, time_resolution
            )
        ...
        s_x = get_s_x_extrapolate(
            ts.index.min(), ts.index.max(), model=model, freq=time_resolution, extrapolate_years=3.0
        )
        logger.info(
            "# of ts: %s, f_model: %s , time_resolution: %s", ts.shape, model, time_resolution
        )
        ...
        l_df_y = [
            # Single ts
            pd.Series(index=pd.date_range("2016-01-01", periods=10, freq="D"), data=np.arange(10)),
            # Multiple ts
            pd.DataFrame(
        ...
        for ts, model, time_resolution in itertools.product(l_df_y, l_models, l_time_resolutions):
            ...
            # and manually check results in logs
            s_x = get_s_x_extrapolate(
                ts.index.min(), ts.index.max(), model=model, freq=time_resolution
            )
            logger.info(
                "# of ts: %s, f_model: %s , time_resolution: %s", ts.shape, model, time_resolution
            )
    ...
    def test_interpolate_df(self):
        ...
        df = pd.concat([df_expected.head(5), df_expected.tail(-6)])
        ...
        df = pd.DataFrame({"date": pd.to_datetime(["2018-08-01", "2018-08-09"]), "y": [1.0, 2.0]})
        df_result = df.pipe(interpolate_df, include_mask=True)
        logger_info("df_result:", df_result)
    ...
    def test_get_mult_sum_stats(self):
        ...
        df_sum = df_in.assign(x=df_in.index, y=(100.0 + df_in.date.dt.weekday + df_in.index))
        # Time series with multiplicative seasonality
        df_mult = df_in.assign(x=df_in.index, y=(100.0 + df_in.date.dt.weekday * df_in.index))
        df_mult2 = df_in.assign(
            x=df_in.index, y=(500.0 + 0.5 * df_in.date.dt.weekday * df_in.index)
        )
        # Add noise
        np.random.seed(1)  # Ensure predictable test results
        df_mult3 = df_mult.assign(y=df_mult.y + np.random.normal(0, 50, df_mult.index.size))
    ...
    def test_is_multiplicative(self):
        ...
        # Time series with additive seasonality
        df_short = df_in.assign(x=df_in.index, y=(100.0 + df_in.date.dt.weekday + df_in.index))
        result = is_multiplicative(df_short)
        self.assertFalse(result)

        logger.info(
            "Test 6a: input too short for linear fit - monthly samples"
            "- more than 4 samples, but < 4 after filtering weight"
        )
        # Time series with additive seasonality
        df_zeroweight = df_in.assign(
            x=df_in.index,
            y=(100.0 + df_in.date.dt.weekday + df_in.index),
            weight=0,
        )
        ...
        logger.info(
            "Test 6b: input too short for linear fit - monthly samples"
